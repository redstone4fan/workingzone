        ...
        stage('Terraform Init'){
            steps {
                    ...
                    withCredentials(
                        ...
                        string(credentialsId: 'tfstat.storage.access.key', variable: 'ARM_ACCESS_KEY')
                        ]
                        )
                        ...
                        sh """           
                            ...
                            terraform init -backend-config="access_key=$ARM_ACCESS_KEY" -force-copy
                            """
                        }
                    }
                }
        ...
        stage('Waiting for Approval'){
            steps {
                timeout(time: 5, unit: 'MINUTES') {
                    input (message: "Confirm to deploy the infrastructure?")
                }
            }
